          ...
          <div className="max-w-[1220px] mx-auto px-4 sm:px-8">
            ...
            {featured && (
              <Link
                ...
                className="group block rounded-2xl p-8 md:p-10 mb-8 transition-all hover:-translate-y-0.5"
                style={{ background: "#fff", border: "1.5px solid #E6E9FF", boxShadow: "0 4px 24px rgba(107,92,231,0.07)" }}
              >
                <div className="flex flex-wrap items-center gap-2 mb-4">
                  ...
                  <span className="text-[12px]" style={{ color: "#8B90A0" }}>{featured.readTime}</span>
                  <span className="text-[12px]" style={{ color: "#8B90A0" }}>
                    &middot; {new Date(featured.date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
                  </span>
                </div>
                <h2 className="text-[22px] md:text-[26px] font-extrabold leading-tight mb-3 transition-colors max-w-[780px]" style={{ color: "#0A0F1E" }}>
                  {featured.title}
                </h2>
                <p className="text-[15px] leading-relaxed mb-5 max-w-[680px]" style={{ color: "#5A6178" }}>
                  {featured.description}
                </p>
                <div className="flex items-center gap-1.5 text-[14px] font-bold" style={{ color: "#6B5CE7" }}>
                  Read article
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {rest.map((post) => (
                  <Link
                    key={post.slug}
                    href={`/blog/${post.slug}`}
                    className="group flex flex-col rounded-2xl p-7 transition-all hover:-translate-y-0.5"
                  ...
                  >
                    ...
                    <h2 className="text-[15px] font-bold mb-2 leading-snug flex-1" style={{ color: "#0A0F1E" }}>
                      {post.title}
                    </h2>
                    <p className="text-[13px] leading-relaxed mb-4" style={{ color: "#5A6178" }}>
                      {post.description}
                    </p>
                    <div className="flex items-center gap-1 text-[13px] font-bold mt-auto" style={{ color: "#6B5CE7" }}>
                      Read article
